...
const server: any = require('https').createServer(
  {
    key: fs.readFileSync(
      '/etc/letsencrypt/live/www.collaudolive.com/privkey.pem'
    ),
    cert: fs.readFileSync(
      '/etc/letsencrypt/live/www.collaudolive.com/cert.pem'
    ),
  },
  app
);
...
io.on('connection', function (socket: any) {
  ...
  socket.on('first_idroom', function (first_idroom: any) {
    ...
    socket.emit(
      'lista_utenti',
      functionListaConference.utentiInConference[indexSingleRoom]
    );
  });
  ...
  socket.on('config_rtmpDestination', function (m: any) {
    ...
    if (typeof m.rtmp != 'string') {
      socket.emit('message', {
        type: 'welcome',
        data: 'rtmp destination setup error.',
      });
      return;
      ...
      let identificativoUtente = functionListaConference.idutentesplit(
        socket._rtmpDestination
      );

      //Inserimento in array generale dei dati del nuovo utente in conference
      let insertArray: any = functionListaConference.userInConferenceVideo(
        Number(numberRoom),
      ...
      let indexSingleRoom: number = functionListaConference.checkPresenzaIdRoom(
        Number(numberRoom)
      );
      console.log('Index room: ' + indexSingleRoom);
      let insertArraySingleRoom: string =
        functionListaConference.utentiInConference[indexSingleRoom];
  ...
  socket.on('config_vcodec', function (m: any) {
    ...
    if (!/^[0-9a-z]{2,}$/.test(m)) {
      socket.emit('message', {
        type: 'fatal',
        data: 'input codec contains illegal character?.',
      });
      return;
    } //for safety
    //-----------------------
  ...
  socket.on('start', function (m: any) {
    ...
    let elementoCoordinate = functionListaConference.checkPresenzaSocketid(
      socket.id
    );
    let arrayRoom =
      functionListaConference.utentiInConference[elementoCoordinate.y];
    ...
    if (functionListaConference.checkpidstream(arrayRoom) != 0) {
      console.log(
        'pidstream: ' + functionListaConference.checkpidstream(arrayRoom)
      );
    ...
    if (socket.id) {
      let socketidCoo = functionListaConference.checkPresenzaSocketid(
        socket.id
      );
      let numberRoom: string =
      ...
      let arrayStream: string = functionListaConference.updateStream(
        socket.id,
        ffmpeg_process.pid
      );
    ...
    ffmpeg_process.stderr.on('data', function (d: any) {
      ...
      //Check BLOCCO VIDEO---------
      if (
        dato_ffmpeg.search('Fatal ERROR: unexpected:rtmp not set yet') != -1
      ) {
        console.log(
          'Fatal ERROR: unexpected:rtmp not set yet --> BLOCCO VIDEO ???'
        );
        socket.broadcast.emit('message', {
          type: 'bloccoVideoRtmp',
          data: 'Fatal ERROR: unexpected:rtmp not set yet',
        });
      }
      ...
      if (dato_ffmpeg.search('Queue input is backward in time') != -1) {
        ...
        socket.broadcast.emit('message', {
          type: 'latenzaVideoRtmp',
          data: 'Queue input is backward in time',
        });
      }
  ...
  socket.on('disconnectStream', function (m: any) {
    ...
    if (ffmpeg_process) {
      ...
      ffmpeg_process.kill('SIGINT');
      console.log(
        'ffmpeg process ended ! (CHIUSURA STREAMING DA DISCONNECTSTREAM'
      );
      ...
      console.warn(
        'killing ffmpeg process attempt failed...(DA DISCONNECTSTREAM")'
      );
