...
=======
exports.getRoom = (req: any, res: any, next: any) => {
...
exports.getProgetti = (req: any, res: any, next: any) => {
    //let id = req.params.id; se dovessimo recuperare il parametro id in GEt dall'URL, altrimenti metto solo il codice per effettuare la query
    
        db.query("SELECT * FROM rappre_prog_gisfo", (err: any, rows: any, fields: any) => {
            if(err){
                res.send('Query error: ' + err.sqlMessage);
            }else{
                res.json(rows);
...
exports.getUtenti = (req: any, res: any, next: any) => {
    ...
    if(!validator.isNumeric(id) || id == 0){
        if(id==''){
            ...
            db.query("SELECT * FROM utenti ORDER BY id DESC", (err: any, rows: any, fields: any) => {
                if(err){
                    res.send('Query error: ' + err.sqlMessage);
                }else{
                    res.json(rows);
        ...
        db.query("SELECT * FROM utenti WHERE id= " + id, (err: any, rows: any, fields: any) => {
            if(err){
                res.send('Query error: ' + err.sqlMessage);
            }else{
                res.json(rows);
