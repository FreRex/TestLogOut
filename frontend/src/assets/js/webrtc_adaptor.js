function WebRTCAdaptor(initialValues) {
  ...
  thiz.bandwidth = 500; //default bandwidth kbps
  ...
  this.updateAudioTrack = function (
    stream,
    streamId,
    mediaConstraints,
    onEndedCallback
    ...
    if (thiz.remotePeerConnection[streamId] != null) {
      var audioTrackSender = thiz.remotePeerConnection[streamId]
        .getSenders()
        .find(function (s) {
      ...
      thiz.remotePeerConnection[streamId]
        ...
        .then(function (response) {
          if (thiz.debug) {
            ...
            /*
            !!! Aggiunto e commentato da Frex
					var btstr = document.createElement("div");
					btstr.innerHTML = streamId;
					document.getElementById("partecipantiaudio").appendChild(btstr);
					
					var btstr = document.createElement("p");
					btstr.innerHTML = streamId;
					document.getElementById("partecipantiaudio").appendChild(btstr);
					*/
            /* 
            !!! Aggiunto da Frex
            //Nuova parte (da prova.php)
            filtroStreamId('1', streamId); 
          */
          }
        })
        .catch(function (error) {
    ...
    wsConn.onmessage = function (event) {
      ...
      if (obj.command == 'start') {
        ...
        if (
          ...
          thiz.closePeerConnection(obj.streamId);
          // console.log('Partecipante uscito:' + obj.streamId);

          /*
          !!! Aggiunta e commentata da Frex
					var btstr = document.createElement("div");
					btstr.innerHTML = "partecipante uscito: " + obj.streamId;
					document.getElementById("partecipantiaudio").appendChild(btstr);
					*/
          /* 
          !!! Aggiunta da Frex
          // Nuova parte per disconnessione partecipante audio(da prova.php)
          filtroStreamId('2', obj.streamId); 
          */
        }
